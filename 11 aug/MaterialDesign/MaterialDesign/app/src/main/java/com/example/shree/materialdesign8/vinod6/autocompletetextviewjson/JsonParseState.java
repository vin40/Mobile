	 ...
	 public List<SuggestGetSetState> getParseJsonWCF(String sName)
		{
		 ...
		 try {
			...
			//URL js = new URL("http://ec2-35-154-210-22.ap-south-1.compute.amazonaws.com/dpts/doctor/ecommerce/onclickfilter/autocomptejson.php?Specility="+temp);
			 URL js = new URL("http://sujwalinsure.com/insure/ws/fetch_statecityarea.php?state_name="+temp);
            ...
            JSONArray jsonArray = jsonResponse.getJSONArray("statelist");
			 Log.d("statelist",jsonArray.toString());
            for(int i = 0; i < jsonArray.length(); i++){
                JSONObject r = jsonArray.getJSONObject(i);
				Log.d("statelist",r.getString("state_id"));
                ListData.add(new SuggestGetSetState(r.getString("state_id"),r.getString("state_name")));
        	}
		} catch (Exception e1) {
